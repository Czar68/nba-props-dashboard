...
async function run(): Promise<void> {
  ...
  const result = await mergeOddsWithPropsWithMetadata(raw);
  ...
  console.log(
    `Legs after EV filter (>= ${(MIN_LEG_EV * 100).toFixed(1)}%): ${legsAfterEvFilter.length} of ${legsAfterEdge.length}`
  );
  ...
  fs.writeFileSync(
    legsOutPath,
    JSON.stringify({ runTimestamp, legs: sortedLegs }, null, 2),
    "utf8"
  );
  ...
  */
  ...
  fs.writeFileSync(
    cardsOutPath,
    JSON.stringify({ runTimestamp, cards: sortedCards }, null, 2),
    "utf8"
  );
