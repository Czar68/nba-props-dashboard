...
import { mergeOddsWithProps, mergeOddsWithPropsWithMetadata } from "./merge_odds";
...
function parseSportsFlag(raw: string | undefined): Sport[] {
  if (!raw || raw.trim() === '') {
    return ['NBA'];
  }
  return raw
...
let sportsArg: string | undefined = undefined;
...
function filterEvPicks(evPicks: EvPick[]): EvPick[] {
  ...
  const standardOnly = evPicks.filter((p) => {
    if (p.isNonStandardOdds && !includeNonStandard) return false;
    return true;
  });
  ...
  const filteredByEdge = standardOnly.filter((p) => meetsUnderdogLegEvFloor(p.legEv));
...
async function main() {
  ...
  const result = await mergeOddsWithPropsWithMetadata(rawProps);
  ...
  const standardStructureIds: UnderdogStructureId[] = ['UD_2P_STD', 'UD_3P_STD', 'UD_4P_STD', 'UD_5P_STD', 'UD_6P_STD'];

  // Define all Flex structures (tiered payout ladders — 1-loss for 3–5, 2-loss for 6–8)
  const flexStructureIds: UnderdogStructureId[] = ['UD_3F_FLX', 'UD_4F_FLX', 'UD_5F_FLX', 'UD_6F_FLX', 'UD_7F_FLX', 'UD_8F_FLX'];

  const allCards: { format: string; card: CardEvResult }[] = [];
  const structureMetricsMap = new Map<string, UnderdogStructureMetrics>();
  ...
  for (const structureId of standardStructureIds) {
    ...
    const viableLegs = sortedEv.filter(leg => meetsUnderdogLegEvFloor(leg.legEv));
    ...
    const targetAcceptedCards = UNDERDOG_TARGET_ACCEPTED_CARDS.standard;
  ...
  for (const structureId of flexStructureIds) {
    ...
    const targetAcceptedCards = UNDERDOG_TARGET_ACCEPTED_CARDS.flex;
    const maxAttempts = getUnderdogMaxAttemptsForStructure({
      structure,
      viableLegCount: viableLegs.length,
      targetAcceptedCards,
...
function writeUnderdogCardsToFile(
  ...
  for (const card of unifiedCards) {
    ...
    const kellyStake = calculateKellyStake(card.cardEv, 10000, sport);
  ...
  logProductionRun(provider, sportsProcessed);
}
