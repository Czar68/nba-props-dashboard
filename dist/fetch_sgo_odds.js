...
exports.fetchSgoPlayerPropOdds = fetchSgoPlayerPropOdds;
...
function mapLeagueToSport(leagueID) {
...
function mapSgoStatIdToCategory(statID, leagueID) {
    ...
    if (league === "NBA") {
        ...
        if (key === "assists")
            return "assists";
        if (key === "pra" || key === "points_rebounds_assists")
            return "pra";
        ...
        if (key === "blocks" || key === "blk")
            return "blocks";
        if (key === "steals" || key === "stl")
            return "steals";
        if (key === "stocks" || key === "steals+blocks")
            return "stocks";
        if (key === "turnovers" || key === "to")
            return "turnovers";
        if (key === "fantasyscore" ||
            key === "fantasy_score" ||
            key === "fantasy_points") {
    ...
    if (league === "NFL") {
        if (key === "passing_yards")
            return "pass_yards";
        if (key === "passing_attempts")
            return "pass_attempts";
        if (key === "passing_completions")
            return "pass_completions";
        if (key === "passing_touchdowns")
            return "pass_tds";
        if (key === "passing_interceptions")
            return "interceptions";
        if (key === "rushing_yards")
            return "rush_yards";
        if (key === "rushing_attempts")
            return "rush_attempts";
        if (key === "rushing+receiving_yards")
            return "rush_rec_yards";
        if (key === "receiving_yards")
            return "rec_yards";
        if (key === "receiving_receptions")
            return "receptions";
    }
    // NHL stats
    if (league === "NHL") {
        ...
        if (key === "saves")
            return "saves";
        if (key === "goals_against" || key === "goalsagainst")
...
async function fetchLeaguePlayerProps(client, leagueID) {
    const results = [];
    ...
    console.log(`fetchSgoPlayerPropOdds: ${leagueID} events length`, events.length);
    const byPlayerAndStat = new Map();
    for (const event of events) {
        const odds = event.odds;
        if (!odds)
            continue;
        const league = event.leagueID ?? leagueID;
    ...
    const filtered = Array.from(byPlayerAndStat.values()).filter((p) => Number.isFinite(p.line) &&
        Number.isFinite(p.overOdds) &&
        Number.isFinite(p.underOdds));
    results.push(...filtered);
    // eslint-disable-next-line no-console
    console.log(`fetchSgoPlayerPropOdds: returning ${filtered.length} ${leagueID} player prop markets from SGO`);
    return results;
}
async function fetchSgoPlayerPropOdds(sports = ['NBA']) {
    ...
    const leagues = sports
        ...
        .filter((league) => ["NBA", "NFL", "NHL", "MLB"].includes(league));
