...
const staticDir = path_1.default.join(__dirname, "..", "web", "dist");
...
async function runPrizePicks(job) {
...
async function runUnderdog(job) {
...
app.get("/api/cards", (req, res) => {
    const siteFilter = (req.query.site || "").toUpperCase();
    const minEv = parseFloat(req.query.minEv) || 0;
    const slipFilter = (req.query.slip || "").toUpperCase();
    const results = [];
    // Read PrizePicks cards
    if (!siteFilter || siteFilter === "PP") {
        const ppData = readJsonFile(path_1.default.join(ROOT, "prizepicks-cards.json"));
        if (ppData && typeof ppData === "object" && ppData !== null) {
            const ppCards = ppData.cards || [];
    ...
    if (!siteFilter || siteFilter === "UD") {
        const udData = readJsonFile(path_1.default.join(ROOT, "underdog-cards.json"));
        if (udData && typeof udData === "object" && udData !== null) {
            const udCards = udData.cards || [];
    ...
    const filtered = results.filter((card) => {
        if (minEv && typeof card.cardEv === "number" && card.cardEv < minEv)
            return false;
        if (slipFilter && card.flexType && card.flexType.toUpperCase() !== slipFilter)
            return false;
        return true;
    });
...
app.get("/api/legs", (req, res) => {
    ...
    if (!siteFilter || siteFilter === "PP") {
        const ppLegs = readJsonFile(path_1.default.join(ROOT, "prizepicks-legs.json"));
    ...
    // Read Underdog legs
    if (!siteFilter || siteFilter === "UD") {
        const udLegs = readJsonFile(path_1.default.join(ROOT, "underdog-legs.json"));
    ...
    const filtered = results.filter((leg) => {
        if (minEdge && typeof leg.edge === "number" && leg.edge < minEdge)
            return false;
        if (leagueFilter && leg.league && String(leg.league).toUpperCase() !== leagueFilter)
            return false;
        return true;
    });
