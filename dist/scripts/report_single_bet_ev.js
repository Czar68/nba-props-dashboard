...
const build_single_bet_inputs_1 = require("../build_single_bet_inputs");
...
function generateReportTable(results) {
    if (results.length === 0) {
...
async function generateTestReport() {
    ...
    console.log();
    // Create test markets
    const testMarkets = (0, build_single_bet_inputs_1.createTestMarkets)();
    console.log(`Generated ${testMarkets.length} test markets`);
    // Convert to SingleBetInput
    const singleBetInputs = (0, build_single_bet_inputs_1.buildSingleBetInputsFromOddsFeed)(testMarkets);
    console.log(`Converted to ${singleBetInputs.length} valid single bet inputs`);
    // Evaluate EV for each
    const evResults = singleBetInputs.map(input => (0, sportsbook_single_ev_1.evaluateSingleBetEV)(input));
...
async function generateLiveReport() {
    ...
    try {
        ...
        const oddsFeedMarkets = (0, build_single_bet_inputs_1.buildOddsFeedMarketsFromExistingData)(sgoMarkets);
        console.log(`Converted to ${oddsFeedMarkets.length} odds feed markets`);
        // Convert to SingleBetInput
        const singleBetInputs = (0, build_single_bet_inputs_1.buildSingleBetInputsFromOddsFeed)(oddsFeedMarkets);
        console.log(`Converted to ${singleBetInputs.length} valid single bet inputs`);
        // Evaluate EV for each
        const evResults = singleBetInputs.map(input => (0, sportsbook_single_ev_1.evaluateSingleBetEV)(input));
        console.log(`Evaluated EV for ${evResults.length} bets`);
